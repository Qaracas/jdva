        ...
        # 2, 3, 4 - Inicio nuevo elemento
            ...
            # 3 - Nivel igual respecto al anterior
                ...
                for (k = _nvl_cambia(s); k < n[0]; k++) {
                    if (k == _nvl_cambia(s) && !(_esmlst(s[1][k], lista[i][j])))
                        json[0] = json[0] _nombre(s[1][k], lista[i][j]);
                    if (_esmlst(s[1][k+1], lista[i][j]))
                        json[0] = json[0] "[";
                    else
